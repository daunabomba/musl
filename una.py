...
def target_install(staging_dir: Path, image_dir: Path, arch="x32"):
    ...
    _get_runner().run(["make", f"DESTDIR={image_dir}", "install-libs"], cwd=repo_root, env=get_build_env(), check=True)
    
    lib_path = image_dir / "usr" / "lib"
    if lib_path.exists():
        _get_runner().run(f"rm -f {lib_path}/*.a {lib_path}/*.o", shell=True, check=True)
